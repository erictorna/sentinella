...
taula$LIVING_SITUATION<-taula$`LIVING SITUATION`
...
variables_socia = c('SEX', 'RELATIONSHIP', 'EMPLOYMENT', 'FINANCING', 'CCAA', 'YEAR', 'PHSYCHIATRY', 'LIVING_SITUATION')

# Convert SENTINEL_F to factor if it's not already
taula[, SENTINEL_F := factor(SENTINEL_F, levels = c('No', 'Sí'))]

# Initialize an empty list to store results
chi_squared_results <- list()

# Perform Chi-squared test for each variable
for (var in variables_socia) {
  # Create a contingency table for each variable with SENTINEL_F
  contingency_table <- table(taula[[var]], taula$SENTINEL_F)
  
  # Perform Chi-squared test
  chi_test <- chisq.test(contingency_table)
  
  # Store the test result (statistic, p-value)
...
# Convert the list of results to a data.table for easier interpretation
chi_squared_summary <- rbindlist(lapply(chi_squared_results, as.data.table), idcol = "variable")

# Sort by Chi-squared statistic or p-value (lower p-values mean more dispersion)
chi_squared_summary <- chi_squared_summary[order(p_value)]

# Show the summary sorted by p-value
print(chi_squared_summary)

```

```{r, warning=FALSE}
variables_socia = c('RELATIONSHIP', 'EMPLOYMENT', 'YEAR', 'PHSYCHIATRY')
...
taula_clean <- na.omit(taula_clean, cols = 'SENTINEL_F')

# Fórmula del procés
matching_formula <- as.formula(paste("SENTINEL_F ~", paste(variables_socia, collapse = " + ")))
